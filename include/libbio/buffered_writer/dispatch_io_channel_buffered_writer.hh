...
namespace libbio {
	...
	protected:
		...
		dispatch_ptr <dispatch_queue_t>	m_io_queue;
	...
	public:
		...
		// FIXME: the errors could be better handled in some other way than throwing in the IO queue.
		dispatch_io_channel_buffered_writer(dispatch_fd_t fd, std::size_t buffer_size, dispatch_queue_t io_queue):
			buffered_writer_base(buffer_size),
			m_io_channel(dispatch_io_create(DISPATCH_IO_RANDOM, fd, io_queue, ^(int error){ if (error) throw std::runtime_error(strerror(error)); })),
			m_io_queue(io_queue, true),
		...
		// FIXME: same as above.
		dispatch_io_channel_buffered_writer(char const *path, int oflag, mode_t mode, std::size_t buffer_size, dispatch_queue_t io_queue):
			buffered_writer_base(buffer_size),
			m_io_channel(dispatch_io_create_with_path(DISPATCH_IO_RANDOM, path, oflag, mode, io_queue, ^(int error){ if (error) throw std::runtime_error(strerror(error)); })),
			m_io_queue(io_queue, true),
			m_writing_buffer(buffer_size, 0),
			m_writing_lock(1)
		{
