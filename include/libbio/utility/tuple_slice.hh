...
namespace libbio {
	...
	auto tuple_slice(t_tuple &tuple)
	{
		if constexpr (t_i < t_lhs)
			return tuple_slice <t_lhs, t_rhs, t_should_add_references, 1 + t_i>(tuple);
		else if constexpr (t_rhs <= t_i)
			return std::tuple <>{};
		else
		{
			typedef std::tuple_element_t <t_i, t_tuple>				input_element_type;
			...
			return std::tuple_cat(
				std::tuple <output_element_type>{std::get <t_i>(tuple)},
				tuple_slice <t_lhs, t_rhs, t_should_add_references, 1 + t_i>(tuple)
			);
		}
	}
	...
	auto tuple_slice(t_tuple &tuple)
	{
		return tuple_slice <t_lhs, std::tuple_size_v <t_tuple>, t_should_add_references, t_lhs>(tuple);
	}
