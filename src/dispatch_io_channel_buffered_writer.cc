...
namespace libbio {
	...
	void dispatch_io_channel_buffered_writer::flush()
	{
		...
		if (byte_count)
		{
			...
			dispatch_ptr dispatch_data(dispatch_data_create(buffer, byte_count, dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{ this->m_writing_lock.unlock(); }));
			dispatch_io_write(*m_io_channel, this->m_output_position, *dispatch_data, *m_io_queue, ^(bool done, dispatch_data_t data, int error) {
				if (error)
