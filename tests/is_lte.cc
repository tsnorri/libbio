...
TEMPLATE_PRODUCT_TEST_CASE(
	"Templated is_lte tests (all combinations)",
	"[template][is_lte]",
	(
		tpg <std::int8_t>::type,
...
)
{
	...
	SECTION("Test zero")
	{
		...
		GIVEN("two zeros")
		{
			WHEN("comparing")
			{
				auto const res(lb::is_lte(left, right));
				THEN("is_lte returns true")
				{
					REQUIRE(true == res);
				}
			}
			...
			{
				auto const res(lb::is_lte(right, left));
				THEN("is_lte returns true")
				{
					REQUIRE(true == res);
				}
...
TEMPLATE_PRODUCT_TEST_CASE(
	"Templated is_lte tests (signed and unsigned)",
	"[template][is_lte]",
	(
		tpg <std::int8_t>::type,
		tpg <std::int16_t>::type,
